...
if uploaded_file:
    ...
    for d in preloaded_doctors:
        d["Supervisor"] = d.pop("Sup") # Rename key to match logic
...
with st.sidebar:
    ...
    if not uploaded_file:
        st.markdown("---")
        st.download_button("📄 Get Template", get_empty_template(), "dental_template.xlsx")
...
# --- TABS ---
tab1, tab2, tab3 = st.tabs(["👥 Team List", "🏥 Clinic Units", "🚀 Schedule"])

with tab1:
    st.info("ℹ️ **Note:** All Residents ('Res') are automatically assigned to Scientific Day on Sunday AM.")
    edited_docs_df = st.data_editor(
        df_docs, num_rows="dynamic", use_container_width=True,
        column_config={
            "Supervisor": st.column_config.CheckboxColumn("Sup?", width="small"),
